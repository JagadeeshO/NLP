...
import { Upload, ArrowLeft, FileText, Loader2, CheckCircle2, AlertCircle, Moon, Sun, Search, LogOut } from 'lucide-react'
...
const AnalyzerPage = () => {
  ...
  const handleFileUpload = async (uploadFile) => {
    ...
    try {
      // Use port 8000 for the unified FastAPI backend
      ...
      if (response.ok && data.success) {
        ...
        setError(data.detail || data.error || 'Failed to analyze file')
      }
    } catch (err) {
    ...
    <div className={`min-h-screen p-6 transition-colors duration-700 ${
      ...
      <div className="fixed top-8 right-8 z-50 flex gap-4">
        <motion.button
            ...
            className={`p-3 rounded-xl backdrop-blur-md transition-all duration-300 flex items-center gap-2 ${
            isDarkMode 
                ? 'bg-red-500/10 text-red-400 border border-red-500/20 hover:bg-red-500/20' 
                : 'bg-white/80 border border-slate-200 text-red-600 shadow-sm'
            }`}
      ...
      >
        <div className="flex flex-col gap-1">
          ...
          <p className={`text-sm ${isDarkMode ? 'text-gray-400' : 'text-slate-500'}`}>
            Welcome back, <span className="font-bold text-indigo-400">{userName}</span> (Authenticated via Biometrics)
          </p>
        </div>
      </motion.header>
              ...
              >
                ...
                <p className={`mb-10 text-lg ${
                  ...
                  <span className={`text-sm mt-2 block ${
                    ...
                    File must contain <strong>product_name</strong> and <strong>review</strong> columns
                  </span>
                </p>
